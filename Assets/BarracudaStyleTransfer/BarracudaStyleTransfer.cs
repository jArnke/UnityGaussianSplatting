    ...
    private void OnRenderImageNormal(RenderTexture source, RenderTexture destination)
    {
        if (useFramerateUpsampling == false)
        {
            ...
            else
            {
                ...
                for(int i = 1; i<psy_iterations; i++){
                    input = new Tensor(tempRT, 3);
                    CustomPinTensorFromTexture(input);
                    Dictionary<string, Tensor> temp2 = new Dictionary<string, Tensor>();
                    temp2.Add("frame", input);
                    worker.Execute(temp2);
                    O = worker.PeekOutput();
                    tempRT = RenderTexture.GetTemporary(targetCamera.pixelWidth, targetCamera.pixelHeight, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.sRGB);
                    CustomTensorToRenderTexture(O, tempRT, 0, 0, Vector4.one, internalSetup.postNetworkColorBias);
                }
